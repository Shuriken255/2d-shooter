...
public class CraftWeaponMenu extends BasicMenu{
	int currentPage;
	
	public CraftWeaponMenu(String layer, int currentPage) {
		super(layer);
	...
	public void buildThis(){
		for(int i = 0; i<5; i++){
			if(currentPage*10+i < Crafter.weaponCraftItems.size()){
				Item item = Crafter.weaponCraftItems.get(i+currentPage*10).getThis();
			...
			if(currentPage*10+i+5 < Crafter.weaponCraftItems.size()){
				Item item = Crafter.weaponCraftItems.get(i+5+currentPage*10).getThis();
		...
		if(currentPage != 0){
			back = new TextButton(layer, 64, 272, 176, 32, 0.6f,
				...
				@Override
				public void action(){
					Menu.changeMainMenu(Menu.CRAFT_WEAPON, currentPage-1);
				}
			};
		} else {
			back = new TextButton(layer, 64, 272, 176, 32, 0.6f,
		...
		if((currentPage+1)*10 < Crafter.weaponCraftItems.size()){
		...
		Layers.addObject(new MainSwitchButton(layer, 256, 16, 240, 32, 0.4f, "CRAFT", Text.CENTER, Text.CENTER){
			@Override
			public void action(){
				Menu.changeMainMenu(2,  currentPage);
				return;
			}
		});
	...
	@Override
	public void build(){
		Layers.addObject(new Frame(layer, 3));
	}
