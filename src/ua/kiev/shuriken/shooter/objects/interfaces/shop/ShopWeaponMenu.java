...
public class ShopWeaponMenu extends BasicMenu{
	 final int currentPage;
	
	public ShopWeaponMenu(String layer, int currentPage) {
		super(layer);
	...
	public void buildThis(){
		for(int i = 0; i<5; i++){
			if(currentPage*10+i < Crafter.weaponShopItems.size()){
				Item item = Crafter.weaponShopItems.get(i+currentPage*10).getThis();
			...
			if(currentPage*10+i+5 < Crafter.weaponShopItems.size()){
				Item item = Crafter.weaponShopItems.get(i+5+currentPage*10).getThis();
		...
		if(currentPage != 0){
			back = new TextButton(layer, 64, 272, 176, 32, 0.6f,
				...
				@Override
				public void action(){
					Menu.changeMainMenu(Menu.SHOP_WEAPON, currentPage-1);
				}
			};
		} else {
			back = new TextButton(layer, 64, 272, 176, 32, 0.6f,
		...
		if((currentPage+1)*10 < Crafter.weaponShopItems.size()){
		...
		Layers.addObject(new MainSwitchButton(layer, 16, 16, 240, 32, 0.4f, "SHOP", Text.CENTER, Text.CENTER){
			@Override
			public void action(){
				Menu.changeMainMenu(0, currentPage);
				return;
			}
		});
	...
	@Override
	public void build(){
		Layers.addObject(new Frame(layer, 1));
	}
